...
class PostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            results.append(torch.cat([per_class, torch.sqrt(per_box_cls)], 1))
...
def make_anchorless_postprocessor(config):
    pre_nms_thresh = config.ANCHORLESS.INFERENCE_TH
    pre_nms_top_n = config.ANCHORLESS.PRE_NMS_TOP_N
    nms_thresh = config.NMS_TH
